(function(ext) {
  $.getScript('https://cdnjs.cloudflare.com/ajax/libs/socket.io/2.1.1/socket.io.js')
    .done(function() {
      ...
      ext.pin = function(csv_name, width, height, magnification, up_left_latitude, up_left_longitude, down_right_latitude, down_right_longitude) {
        let command = "pin:" + csv_name + ":" + width + ":" + height + ":" + magnification + ":" + up_left_latitude + ":" + up_left_longitude + ":" + down_right_latitude + ":" + down_right_longitude;
        socket.emit("from_client", JSON.stringify({roomId: roomId, command: command}));
      }
      ...
      var locale = {
        ja: {
          ...
          pin: 'ピンを立てる。位置データを %s 、横を %n 、縦を %n 、拡大倍率を %n 、左上緯度を %n 、左上経度を %n 右下緯度を %n 右下経度を %n',
        ...
        en: {
          ...
          pin: 'stand pins at potision: %s width: %n height: %n magnification: %n up-left (latitude: %n longitude: %n ) down-right (latitude: %n longitude: %n )',
      ...
      var descriptor = {
        blocks: [
          ...
          [' ', locale[lang].pin, 'pin', 'potision_data.csv', 257, 257, 2, 46.317, 126.738, 28.613, 149.238],
