(function(ext) {
  $.getScript('https://cdnjs.cloudflare.com/ajax/libs/socket.io/2.1.1/socket.io.js')
    .done(function() {
      ...
      ext.ar_game = function(x, y, z, model, color, direction) {
        let command = "ar_game:" + x + ":" + y + ":" + z + ":" + model + ":" + color + ":" + direction;
        socket.emit("from_client", JSON.stringify({roomId: roomId, command: command}));
      }
      ...
      var locale = {
        ...
        en: {
          ...
          ar_game: 'spawn character at x: %n y: %n z: %n model: %m.model color: %m.color direction: %m.direction',
